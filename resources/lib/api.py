...
def purge_last_viewed(plugin, usr, pwd):
    ...
    headers = _add_auth_token(plugin, usr, pwd, _ARTETV_HEADERS)
    r = requests.patch(url, data={}, headers=headers, timeout=10)
    return r.status_code
...
def collection(kind, collection_id, lang):
    ...
    return hof.flat_map(lambda subCollections: subCollections.get('videos', []), sub_collections)
...
def get_and_persist_token(plugin, username='', password='', headers=None):
    ...
    if token_idx in cached_token:
        xbmc.log(f"\"{username}\" already authenticated to Arte TV : {cached_token[token_idx]['access_token']}")
...
def authenticate_in_arte(plugin, username='', password='', headers=None):
    ...
    if error or not r or r.status_code != 200:
        ...
        xbmc.log(f"Unable to authenticate to Arte TV : {error if error else r.text}")
        return None
    return r.json(object_pairs_hook=OrderedDict)
...
def persist_token_in_arte(plugin, tokens, headers):
    ...
    custom_token = None
    try:
        custom_token = requests.get(url,params=params, headers=headers, cookies=cookies, timeout=10)
    except requests.exceptions.ConnectionError as err:
        error = err
    if error or not custom_token or custom_token.status_code != 200:
        ...
        xbmc.log(f"Unable to persist Arte TV token {tokens['access_token']}. Step 1/2: {error if error else custom_token.text}")
        return False
    ...
    if error or not login or login.status_code != 200:
        plugin.notify(msg=plugin.addon.getLocalizedString(30020), image='warning')
        xbmc.log(f"Unable to persist Arte TV token {tokens['access_token']}. Step 2/2: {error if error else login.text}")
        return False
